import type { SeedLocale } from "./website-content";
...
function buildSummary(title: string, locale: SeedLocale) {
  return locale === "ar"
...
function buildBody(title: string, locale: SeedLocale) {
  return locale === "ar"
...
export const SECTOR_SEEDS: SectorSeed[] = charterSectors.map((item) => ({
  slug: item.slug,
  ...
  ar: {
    ...
    outputs: [
      "قرارات تنفيذية للمئة يوم",
      "خطة قصيرة ومتوسطة المدى",
      "مؤشرات أداء ومخاطر",
...
export function getSectorEntryData(
  sector: SectorSeed,
  locale: SeedLocale,
  order: number
) {
  const localized = sector[locale];
  return {
    slug: sector.slug,
