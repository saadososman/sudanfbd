...
import { configurePublicPermissions } from "./bootstrap/public-permissions";
...
export default {
  ...
  async bootstrap({ strapi }: { strapi: Core.Strapi }) {
    ...
    try {
      ...
      strapi.log.warn(
        `Public permissions bootstrap skipped: ${error instanceof Error ? error.message : "unknown error"}`
      );
    }
  }
};
